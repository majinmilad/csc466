...
The aim of my adventurer profiler was to capture user engagement behavior and be able to categorize adventurers into one of a few engagement styles. In order to build adventurer personas in an unsupervised manner, I used k-means clustering via the scikit-learn implementation. I developed an adventurer feature table with one row per adventurer and filled it with view count, seconds watched, watch completion, days since active, publisher count, playlist count, app opens, and subscription cancellations. I used StandardScaler where appropriate and for all heavy-tailed counts I log1p to compress extreme values and reduce dominance of outliers. Via an elbow plotting of different `K` values for the algorithm, I arrived at `K` = 5. The elbow curve was relatively smooth, but the reduction in inertia was clear around a K value of 5.
...
4. The Uninterested (13.2% of users)
   ...
   This group has abismal engagement metrics and show as users who have dabbled with the platform but show little interest in its offerings or have not had the opportunity to properly explore it. They interaction is not non-existant, however. With appreciable app opens and views, they have tried the platform out, and so this leans most likely towards users who have low interest. Their age is, unsuprisingly, the highest of all the groups (is that ageist?) and again correlates with the commonly seen trend of older users having lower engagement. They represent a tenth of the platform base and read as at high-risk of churning.
...
5. The I-Forgot-To-Cancel-My-Subscription Crowd (1.7% of users)
   ...
   This cluster stood out as an anomly grouping. With absolutely zero viewing behavior, but existant subscriptions and app openings, the group seems to be most likely a clustering of outliers, and legitamte users who minimally explored the platform but never interacted.
...
In the visual above, you can see a charting of users' average completion rates for individual items of content versus their overall engagement on the app (measured as length of time spent viewing). When the data points are color coded for their cluster, they show a clear grouping of the for the clusters in regards to these features, with a sort of "decision boundary" between them. This use of clustering to identify behavior patterns and defining user types can be advantageously incorporated into recommendation systems. The type of persona a user shows to be can help inform the recommendation system used for them. For instance, if a user shows to be a heavy explorer, the system can put added weight to diversifying the content suggested to them. You could also implement personas in recommending by treating it as an actual feature. Apart from recommendation, this type of clustering might prove useful for churn detection, as you could identify a persona type which may be susceptible to churning. In our own clustering here, we identified one such possible type, in the form of our I-Forgot-To-Cancel-My-Subscription persona, who appear to be strong candidates to churn (if they ever do, in fact, remember that they have active subscriptions).
